---
title: "`claude -p` 를 꼭 쓸 줄 알아야하는 이유에 대하여"
...
이게 별 거 아닌 것 같지만, **이 한 줄이 할 수 있는 일**이 내 세팅을 완전히 업그레이드 시켜줬다.
...
이게 대단한 이유는, 지금까지 "코드로 풀기 어려웠던" 문제들이 **한 줄**로 풀리기 때문이다.
...
여기까지는 "편하다" 정도일 수 있는데, 진짜 큰 건 다음 이야기다.
...
Claude Code가 "AI 코딩 도구"로만 알려져 있는데, 진짜 물건은 `-p` **플래그**다. 이거 쓰기 시작하면 "사람이 쳐야 하는 일"과 "프로그램이 할 수 있는 일" 사이 경계가 움직인다. 생각보다 꽤 많이...